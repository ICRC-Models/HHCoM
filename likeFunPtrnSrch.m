...
for aV = 1 : length(ageVec)
    ...
    ageInds = toInd(allcomb([2:6,10] , 1 : viral , 1 : hpvTypes , 1 : hpvStates , 1 : periods , ...
        1 , a , 1 : risk));
    cinPos2007(aV) = sum(popVec((2007 - startYear) * stepsPerYear , hpvInds))...
        ./ sum(popVec((2007 - startYear) * stepsPerYear , ageInds)) * 100;
end
% 
pPos = [cinPos2007];
N = [cinPos2007_obs(: , 3)];
nPos = [cinPos2007_obs(: , 2)];
% 
...
%         2 , a , 1 : risk))];
%     cinPos2007(a - 3) = (sum(popVec((2002 - startYear) * stepsPerYear , cinInds)))...
...
for a = 6 : 11
    ...
    ageInds = toInd(allcomb(1 : disease , 1 : viral , 1 : hpvTypes , 1 : hpvStates , 1 : periods , ...
        2 , a , 1 : risk));
    hpv2000(a - 3) = sum(popVec((2000 - startYear) * stepsPerYear , hpvInds))...
        ./ sum(popVec((2000 - startYear) * stepsPerYear , ageInds)) * 100;
end
hpv2000 = hpv2000(3:8, :);
pPos = [pPos ; hpv2000];
N = [N; hpv_2000_obs(: , 3)];
nPos = [nPos; hpv_2000_obs(: , 2)];
