...
# RESTRICT TO limited set of scenarios

# VMMC Scale-Up

version <- c(main_path)
names(version) <- c("main")
...
men_daly_list <- vector(mode = "list")
men_daly_list_summary <- vector(mode = "list")
...
for (v in names(version)) {  
  ...
  for (x in scenarios) {
      ...
      for (d in names(discount_rate)) {
        ...
        men_daly_list[[length(men_daly_list) + 1]] <- daly
        men_daly_list_summary[[length(men_daly_list_summary) + 1]] <- summary
        
        names(men_daly_list)[length(men_daly_list)] <- paste(x, d, v, sep = ".")
        names(men_daly_list_summary)[length(men_daly_list_summary)] <- paste(x, d, v, sep = ".")
