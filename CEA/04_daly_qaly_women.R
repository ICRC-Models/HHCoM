...
women_daly_list <- vector(mode = "list")
women_daly_list_summary <- vector(mode = "list")
...
for (v in names(version)) {  
  ...
  for (x in scenarios) {
      ...
      for (d in names(discount_rate)) {
        ...
        women_daly_list[[length(women_daly_list) + 1]] <- daly
        women_daly_list_summary[[length(women_daly_list_summary) + 1]] <- summary
        
        names(women_daly_list)[length(women_daly_list)] <- paste(x, d, v, sep = ".")
        names(women_daly_list_summary)[length(women_daly_list_summary)] <- paste(x, d, v, sep = ".")
    }
  }
}
