...
# Multiply number of men circumcised by cost of circumcision
...
for (x in 1:3) {
  ...
  vmmc <- as.data.frame(read.csv(paste0(main_path,"Scenario",x,"/Raw_VMMC_male_ages15-79.csv"), header=F)[,-1]) %>% 
    setNames(paste0("s",-2:25)) %>% 
    mutate(year=1925:2060) %>%
    filter(year>=2020) %>%
    select(year, everything()) %>%
  ...
  assign(paste0("vmmc_",x), vmmc)
  
}

rm(vmmc)

# Cost per person in population

vmmc_scen1 <- as.data.frame(
  (vmmc_1[,-1] * costs["circumcision"]) / pop_scen1[,-1] )  %>% 
  # Recreate mean. min, max
  mutate(mean=rowMeans(.[,4:28]),
         min=apply(.[,4:28],1,FUN=min),
         max=apply(.[,4:28],1,FUN=max)) %>% 
...
vmmc_scen2 <- as.data.frame(
  (vmmc_2[,-1] * costs["circumcision"]) / pop_scen2[,-1] )  %>% 
  # Recreate mean. min, max
  mutate(mean=rowMeans(.[,4:28]),
         min=apply(.[,4:28],1,FUN=min),
         max=apply(.[,4:28],1,FUN=max)) %>% 
...
vmmc_scen3 <- as.data.frame(
  (vmmc_3[,-1] * costs["circumcision"]) / pop_scen3[,-1] )  %>% 
  # Recreate mean. min, max
  mutate(mean=rowMeans(.[,4:28]),
         min=apply(.[,4:28],1,FUN=min),
         max=apply(.[,4:28],1,FUN=max)) %>% 
