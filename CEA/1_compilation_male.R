...
#FEMALE (later - combined)

for (x in 1:3) {

m <- read_excel(paste0(model_outputs,"Scenario",x,"/HIV_mortality_males_aged15-79.xlsx"), 
                          sheet="HIVmortM", col_names=F) %>% 
...
for (x in 1:3) {
  ...
  i <- read_excel(paste0(model_outputs,"Scenario",x,"/HIV_incidence_males_aged15-79.xlsx"), 
                  sheet="HIVincM", col_names=F) %>% 
    rename(year=1, mean=2, min=3, max=4) %>% 
    mutate(cum_mean=cumsum(mean), 
           cum_min=cumsum(min),
           cum_sum=cumsum(max)) %>% 
...
for (x in 1:3) {
  ...
  m <- read_excel(paste0(model_outputs,"Scenario",x,"/HIV_prevalence_males_aged15-79_onART.xlsx"), 
                  sheet="HIVprevM", col_names=F) %>% 
    rename(year=1, mean=2, min=3, max=4)  
...
for (x in 1:3) {
  ...
  m <- read_excel(paste0(model_outputs,"Scenario",x,"/HIV_prevalence_males_aged15-79_CD4 below200 noART.xlsx"), 
                  sheet="HIVprevM", col_names=F) %>% 
    rename(year=1, mean=2, min=3, max=4)  
...
for (x in 1:3) {
  ...
  m <- read_excel(paste0(model_outputs,"Scenario",x,"/HIV_prevalence_males_aged15-79_CD4 200-350 noART.xlsx"), 
                  sheet="HIVprevM", col_names=F) %>% 
    rename(year=1, mean=2, min=3, max=4)  
...
for (x in 1:3) {
  ...
  m <- read_excel(paste0(model_outputs,"Scenario",x,"/HIV_prevalence_males_aged15-79_CD4 350plus noART.xlsx"), 
                  sheet="HIVprevM", col_names=F) %>% 
    rename(year=1, mean=2, min=3, max=4)  
