function [deaths, newCC, ccHealthState, symptomatic, treatment, screening] = debugTreatment_historicalSim(vaxResultInd , sceNum , fileNameNums, fileInds, deaths, ccHealthState, newCC, symptomatic, treatment, screening)
...
lastYear = 2021;  % ***SET ME***: last year of simulation (use 2122 for SA screening analysis)
...
% SYMPTOMATIC CASES **************************************
    ...
    for x = 1 : 3
        for a = 1 : age
            for p = 1 : 3
                symptomatic(:, a, x, p, j) = sum(sum(sum(sum(sum(sum(sum(vaxResult{n}.ccSymp(:, :, :, :, x, :, a, p),2),3),4),5),6),7),8);
            end 
        end
    end 
...
% TREATMENT ************************************************
    ...
    for x = 1 : 3
        for a = 1 : age
                for p = 1 : 3
                    treatment(:, a, x, p, j) = sum(sum(sum(sum(sum(sum(sum(vaxResult{n}.ccTreat(:, :, :, :, x, :, a, p),2),3),4),5),6),7),8); 
                end 
        end 
    end 
