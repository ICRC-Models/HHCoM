...
ageGroup = {'15 - 19' , '20 - 24' , '25 - 29' ,...
    ...
    '60 - 64' , '65 - 69' , '70 - 74'};
ageEndVec = {11 , 10 , 13};
genVecLabsls = {'Male' , 'Female' , 'Male+Female Combined'};
genVec = {1 , 2 , 1:2};
...
for gInd = 1 : length(genVec)
    ...
    for i = 1 : size(hivModel , 3)
        hold all;       
        errorbar(1 : ageEndVec{gInd} , hivPrevs(((i-1)*ageEndVec{gInd} : ageEndVec{gInd}*i) , 1) , ...
            hivPrevs(((i-1)*ageEndVec{gInd} : ageEndVec{gInd}*i) , 2) , ...
            'rs' , 'LineWidth' , 1.5);
        hold all;
        plot(1 : ageEndVec{gInd} , mean((squeeze(hivModel(: , : , i)) .* 100),1)' , 'k-' , ...
            1 : ageEndVec{gInd} , min((squeeze(hivModel(: , : , i)) .* 100),[],1)' , 'k--' , ...
            1 : ageEndVec{gInd} , max((squeeze(hivModel(: , : , i)) .* 100),[],1)' , 'k--' , 'LineWidth' , 1.5);
        set(gca , 'xtickLabel' , ageGroup); set(gca , 'xtick' , 1 : length(ageGroup) , 'xtickLabel' , ageGroup);
    end
    xlabel('Age Group'); ylabel('HIV Prevalence (%)'); title(gen{gInd});
    xlim([2000 2020]); 
    grid on;
    if (gInd == 1) || (gInd == 2)
