...
% fileInds = {'0_1272' , '0_1691' , '0_1909' , '0_2130' , '0_2285' , ...
%     '0_2362' , '0_2410' , '0_2572' , '0_3195' , '0_3198' , '0_3402' , ...
%     '0_3430' , '0_3509' , '0_3599' , '0_3662' , '0_3953' , '0_4049' , ...
%     '0_4235' , '0_4659' , '0_4928' , '0_4928' , '0_4936' , '0_5065' , ...
%     '0_5103' , '0_5227' , '0_5287' , '0_5316' , '0_5340' , '0_5382' , ...
%     '0_6054' , '0_6277' , '0_6466' , '0_6831' , '0_6996' , '0_7006' , ...
%     '0_7206' , '0_7222' , '0_7382' , '0_776' , '0_7971' , '0_8037' , ...
%     '0_8083' , '0_8714' , '0_9115' , '0_9524' , '0_9556' , '0_9933'};    % 22Apr20Ph2V4
...
mortM_hivNeg = zeros(nRuns , length(mortYearVec));
mortM_hivPos = mortM_hivNeg;
mortM_hivPosART = mortM_hivNeg;
mortM_hivPosNoART = mortM_hivNeg;
mortF_hivNeg = mortM_hivNeg;
mortF_hivPos = mortM_hivNeg;
mortF_hivPosART = mortM_hivNeg;
mortF_hivPosNoART = mortM_hivNeg;
% ART and circumcision
artCovM = zeros(nRuns , length([startYear : timeStep : currYear]));
artCovF = artCovM;
artCovAge = zeros(nRuns , age , length([startYear : timeStep : currYear]));
ageVecCirc = {4 , 5 , [6:10] , [11:age]}; % Ages: (15-19), (20-24), (25-49), (50+)
ageVecCirc_length = length(ageVecCirc);
circProp = zeros(nRuns , ageVecCirc_length , length([startYear : timeStep : currYear]));
...
for j = 1 : nRuns
    % Load results
    pathModifier = ['toNow_22Apr20Ph2V11_baseVax057_baseScreen_baseVMMC_fertDec042-076_2020ARTfxd_trackCD4-Discont_discontFxd_DoART_S1_' , fileInds{j}];
    ...
    for t = 1 : length(mortYearVec)
            ...
            popTotF_hivNeg = annlz(sum(popVec(mortTimeSpan , popIndsF_hivNeg) , 2)) ./ stepsPerYear; 
            mortF_hivNeg(j , t) = (annlz(sum(sum(hivDeaths(mortTimeSpan , 1 : 2 , 2 , 4 : 13), 2), 4)) + ...
                annlz(sum(sum(sum(deaths(mortTimeSpan , 1 : 2 , 2 , 4 : 13), 2), 3), 4))) ...
                ./ popTotF_hivNeg .* 100000;
            popIndsF_hivPos = toInd(allcomb(3 : disease , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates , 1 : endpoints , ...
                1 : intervens , 2 , 4 : 13 , 1 : risk));
            popTotF_hivPos = annlz(sum(popVec(mortTimeSpan , popIndsF_hivPos) , 2)) ./ stepsPerYear;
            mortF_hivPos(j , t) = (annlz(sum(sum(hivDeaths(mortTimeSpan , 3 : disease , 2 , 4 : 13), 2), 4)) + ...
                annlz(sum(sum(sum(deaths(mortTimeSpan , 3 : disease , 2 , 4 : 13), 2), 3), 4))) ...
                ./ popTotF_hivPos .* 100000;
            
            popIndsF_hivPosART = toInd(allcomb(8 , 6 , 1 : hpvVaxStates , 1 : hpvNonVaxStates , 1 : endpoints , ...
                1 : intervens , 2 , 4 : 13 , 1 : risk));
            popTotF_hivPosART = annlz(sum(popVec(mortTimeSpan , popIndsF_hivPosART) , 2)) ./ stepsPerYear;
            mortF_hivPosART(j , t) = (annlz(sum(sum(hivDeaths(mortTimeSpan , 8 , 2 , 4 : 13), 2), 4)) + ...
                annlz(sum(sum(sum(deaths(mortTimeSpan , 8 , 2 , 4 : 13), 2), 3), 4))) ...
                ./ popTotF_hivPosART .* 100000;
            popIndsF_hivPosNoART = toInd(allcomb(3 : 7 , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates , 1 : endpoints , ...
                1 : intervens , 2 , 4 : 13 , 1 : risk));
            popTotF_hivPosNoART = annlz(sum(popVec(mortTimeSpan , popIndsF_hivPosNoART) , 2)) ./ stepsPerYear;
            mortF_hivPosNoART(j , t) = (annlz(sum(sum(hivDeaths(mortTimeSpan , 3 : 7 , 2 , 4 : 13), 2), 4)) + ...
                annlz(sum(sum(sum(deaths(mortTimeSpan , 3 : 7 , 2 , 4 : 13), 2), 3), 4))) ...
                ./ popTotF_hivPosNoART .* 100000;
            
            popIndsM_hivNeg = toInd(allcomb(1 : 2 , 1 , 1 : hpvVaxStates , 1 : hpvNonVaxStates , 1 : endpoints , ...
                1 : intervens , 1 , 4 : 13 , 1 : risk));
            popTotM_hivNeg = annlz(sum(popVec(mortTimeSpan , popIndsM_hivNeg) , 2)) ./ stepsPerYear;
            mortM_hivNeg(j , t) = (annlz(sum(sum(hivDeaths(mortTimeSpan , 1 : 2 , 1 , 4 : 13), 2), 4)) + ...
                annlz(sum(sum(sum(deaths(mortTimeSpan , 1 : 2 , 1 , 4 : 13), 2), 3), 4))) ...
                ./ popTotM_hivNeg .* 100000;
            popIndsM_hivPos = toInd(allcomb(3 : disease , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates , 1 : endpoints , ...
                1 : intervens , 1 , 4 : 13 , 1 : risk));
            popTotM_hivPos = annlz(sum(popVec(mortTimeSpan , popIndsM_hivPos) , 2)) ./ stepsPerYear;
            mortM_hivPos(j , t) = (annlz(sum(sum(hivDeaths(mortTimeSpan , 3 : disease , 1 , 4 : 13), 2), 4)) + ...
                annlz(sum(sum(sum(deaths(mortTimeSpan , 3 : disease , 1 , 4 : 13), 2), 3), 4))) ...
                ./ popTotM_hivPos .* 100000;
            
            popIndsM_hivPosART = toInd(allcomb(8 , 6 , 1 : hpvVaxStates , 1 : hpvNonVaxStates , 1 : endpoints , ...
                1 : intervens , 1 , 4 : 13 , 1 : risk));
            popTotM_hivPosART = annlz(sum(popVec(mortTimeSpan , popIndsM_hivPosART) , 2)) ./ stepsPerYear;
            mortM_hivPosART(j , t) = (annlz(sum(sum(hivDeaths(mortTimeSpan , 8 , 1 , 4 : 13), 2), 4)) + ...
                annlz(sum(sum(sum(deaths(mortTimeSpan , 8 , 1 , 4 : 13), 2), 3), 4))) ...
                ./ popTotM_hivPosART .* 100000;
            popIndsM_hivPosNoART = toInd(allcomb(3 : 7 , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates , 1 : endpoints , ...
                1 : intervens , 1 , 4 : 13 , 1 : risk));
            popTotM_hivPosNoART = annlz(sum(popVec(mortTimeSpan , popIndsM_hivPosNoART) , 2)) ./ stepsPerYear;
            mortM_hivPosNoART(j , t) = (annlz(sum(sum(hivDeaths(mortTimeSpan , 3 : 7 , 1 , 4 : 13), 2), 4)) + ...
                annlz(sum(sum(sum(deaths(mortTimeSpan , 3 : 7 , 1 , 4 : 13), 2), 3), 4))) ...
                ./ popTotM_hivPosNoART .* 100000;
    end
        ...
        hivAllIndsM = toInd(allcomb(3 : 7 , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates, ...
            1 : endpoints , 1 : intervens , 1 , 4 : age , 1 : risk));
        artCovF(j , :) = (sum(popVec(: , artIndsF) , 2) ./ (sum(popVec(: , hivAllIndsF) , 2) + sum(popVec(: , artIndsF) , 2))) .* 100;
        artCovM(j , :) = (sum(popVec(: , artIndsM) , 2) ./ (sum(popVec(: , hivAllIndsM) , 2) + sum(popVec(: , artIndsM) , 2))) .* 100;
...
plot(totPopSize_dObs(: , 1)' , mean(popSize,1) , 'k-' , 'LineWidth' , 1.5); % , ...
%     totPopSize_dObs(: , 1) , min(popSize,[],1)' , 'k--' , ...
%     totPopSize_dObs(: , 1) , max(popSize,[],1)' , 'k--' , 'LineWidth' , 1.5);
hold all;
x2 = [totPopSize_dObs(: , 1)' , fliplr(totPopSize_dObs(: , 1)')];
...
legend('(Statistics SA) Observed KZN: mean, 2SD' , 'Modeled KZN: mean' , 'Modeled KZN: range' , 'Location' , 'SouthEast');
grid on;
...
for g = 1 : gender
    ...
    for a = 4 : 10
        ...
        if a <= 11            
            ...
            hold on;
            plot(hivYearVec' , mean((squeeze(hivModel(: , a-3 , :)) .* 100),1) , 'k-' , 'LineWidth' , 1.5); %...
            %    hivYearVec , min((squeeze(hivModel(: , a-3 , :)) .* 100),[],1)' , 'k--' , ...
            %    hivYearVec , max((squeeze(hivModel(: , a-3 , :)) .* 100),[],1)' , 'k--' , 'LineWidth' , 1.5);
            hold all;
            x2 = [hivYearVec' , fliplr(hivYearVec')];
            inBetween = [max((squeeze(hivModel(: , a-3 , :)) .* 100) , [] , 1) , fliplr(min((squeeze(hivModel(: , a-3 , :)) .* 100) , [] , 1))];
            h = fill(x2 , inBetween , 'k');
            h.FaceColor = 'k';
            h.EdgeColor = 'k';
            h.FaceAlpha = 0.3;
        ...
        xlabel('Year'); ylabel('HIV Prevalence (%)'); title([gen{g} , ' aged ' , ageGroup{a-3}]) % , ' HIV Prevalence'])
        xlim([2000 2020]); 
        if g == 1 
            ylim([0 60]);
        elseif g == 2
...
hivRawHist(:,:,1) = hivPrevM_dObs(: , 4:5);
hivRawHist(:,:,2) = hivPrevF_dObs(: , 4:5);
% 2010-2016 raw AC data
hivPrevF_valR = [114 1227; 289 920; 375 704; 303 512; 265 491; 187 438; 184 569; ...
                95 1053; 255 805; 316 611; 281 458; 245 453; 193 446; 166 484; ...
                97 928; 201 656; 253 498; 210 358; 211 359; 154 340; 143 365; ...
                119	1275; 275 810; 308 600; 331 510; 295 453; 206 419; 175 422; ...
                114	1099; 264 764; 321 618; 336 537; 299 458; 230 424; 189 392; ...
                171	1554; 360 1055; 468 867; 506 782; 397 614; 332 589; 289 583; ...
                107	1144; 258 772; 348 664; 400 634; 310 463; 258 421; 217 432];
hivPrevM_valR = [17 1060; 52 544; 69 238; 79 170; 77 148; 58 139; 48 131; ...
                17 917; 41 511; 57 260; 77 173; 70 158; 49 118; 49 132; ...
                23 837; 39 395; 52 209; 62 157; 63 127; 50 97; 34 103; ...
                39 1128; 47 487; 74 248; 85 180; 95 150; 63 122; 46 115; ...
                28 975; 51 430; 80 226; 89 192; 73 142; 79 133; 45 111; ...
                53 1343; 43 598; 94 340; 122 293; 89 171; 88 167; 69 155; ...
                43 956; 34 424; 65 238; 85 202; 76 148; 65 127; 48 92];
hivRaw(:,:,1) = [hivRawHist(:,:,1); hivPrevM_valR];
hivRaw(:,:,2) = [hivRawHist(:,:,2); hivPrevF_valR];
for i = 1 : length(hivYearVec)
    for g = 1 : gender
...
for g = 1 : gender
    subplot(1,2,g)
    plot(hivYearVec' , hivData(:,:,g) , 'bo');
    %hold on;
    %plot([2010 : 2016]' , hivDataAlt((6:end-1),:,g) , 'co');
    hold on;
    plot(hivYearVec' , mean(squeeze(hivPrev(: , g , :)),1) , 'k-' , 'LineWidth' , 1.5); ...
    hold all;
    x2 = [hivYearVec' , fliplr(hivYearVec')];
...
hold on;
plot(prevYears2 , hivPrev_val , 'bo');
hold on;
plot(hivYearVec2 , mean(hivPrevTot(: , :),1) , 'k-' , 'LineWidth' , 1.5); %...
%    hivYearVec2 , min(hivPrevTot(: , :),[],1)' , 'k--' , ...
%    hivYearVec2 , max(hivPrevTot(: , :),[],1)' , 'k--' , 'LineWidth' , 1.5); % , 'Color' , [0.9290, 0.6940, 0.1250])
hold all;
x2 = [hivYearVec2 , fliplr(hivYearVec2)];
...
errorbar(hivPrev_obs(: , 1) , hivPrev_obs(: , 2) , ...
        ...
        'bs' , 'LineWidth' , 1.5); % , 'Color' , [0.9290, 0.6940, 0.1250])
hold on;
plot(hivYearVec2 , mean(hivPrevTot2(: , :),1)' , 'k-' , 'LineWidth' , 1.5); %...
hold all;
x2 = [hivYearVec2 , fliplr(hivYearVec2)];
...
xlabel('Year'); ylabel('HIV Prevalence (%)');
xlim([2000 2020]); ylim([0 50]); grid on;
legend('(HSRC SABSSM) Observed KZN, ages 25+: 95% CI' , 'Modeled KZN, ages 25-79: mean' , 'range');
...
errorbar(hivPrev_obs(: , 1) , hivPrev_obs(: , 2) , ...
        ...
        'bs' , 'LineWidth' , 1.5); % , 'Color' , [0.9290, 0.6940, 0.1250])
hold on;
plot(hivYearVec2 , mean(hivPrevTot3(: , :),1)' , 'k-' , 'LineWidth' , 1.5); %...
hold all;
x2 = [hivYearVec2 , fliplr(hivYearVec2)];
...
xlabel('Year'); ylabel('HIV Prevalence (%)');
xlim([2000 2020]); ylim([0 50]); grid on;
legend('(HSRC SABSSM) Observed KZN, ages 50+: 95% CI' , 'Modeled KZN, ages 50-79: mean' , 'range');
...
for g = 1 : gender
    ...
    xlim([2000 2020]); ylim([0 15]); title(titlesHIVinc{(1 + ((g-1)*3))}); grid on;
    legend('(Vandormael, 2019) Observed KZN: 95% CI' , 'Modeled KZN: mean' , ...
        'range');
    ...
    errorbar(hivIncYoung_obs(: , 1 , g) , hivIncYoung_obs(: , 2 , g) , ...
        ...
        'bs' , 'LineWidth' , 1.5); % , 'Color' , [0.9290, 0.6940, 0.1250])
    hold on;
    plot(hivIncYearVec , mean(squeeze(hivIncYoung(: , g , :)),1) , 'k-' , 'LineWidth' , 1.5); %
    hold all;
    x2 = [hivIncYearVec , fliplr(hivIncYearVec)];
    ...
    xlabel('Year'); ylabel('HIV incidence per 100'); 
    xlim([2000 2020]); ylim([0 15]); title(titlesHIVinc{(2 + ((g-1)*3))}); grid on;
    legend('(Vandormael, 2019) Observed KZN: 95% CI' , 'Modeled KZN: mean' , ...
        'range');
    ...
    errorbar(hivIncOlder_obs(: , 1 , g) , hivIncOlder_obs(: , 2 , g) , ...
        ...
        'bs' , 'LineWidth' , 1.5); % , 'Color' , [0.9290, 0.6940, 0.1250])
    hold on;
    plot(hivIncYearVec , mean(squeeze(hivIncOlder(: , g , :)),1) , 'k-' , 'LineWidth' , 1.5); %...
    hold all;
    x2 = [hivIncYearVec , fliplr(hivIncYearVec)];
    ...
    xlabel('Year'); ylabel('HIV incidence per 100'); 
    xlim([2000 2020]); ylim([0 15]); title(titlesHIVinc{(3 + ((g-1)*3))}); grid on;
    legend('(Vandormael, 2019) Observed KZN: 95% CI' , 'Modeled KZN: mean' , ...
        'range');
end
...
figure;
subplot(1 , 3 , 1)
plot(mortYearVec , mean(squeeze(mortM_hivPosART(: , :)),1)' , 'k-' , ...
    mortYearVec , min(squeeze(mortM_hivPosART(: , :)),[],1)' , 'k--' , ...
    mortYearVec , max(squeeze(mortM_hivPosART(: , :)),[],1)' , 'k--' , 'LineWidth' , 1.5);
hold all;
plot(mortYearVec , mean(squeeze(mortM_hivPosNoART(: , :)),1)' , 'b-' , ...
    mortYearVec , min(squeeze(mortM_hivPosNoART(: , :)),[],1)' , 'b--' , ...
    mortYearVec , max(squeeze(mortM_hivPosNoART(: , :)),[],1)' , 'b--' , 'LineWidth' , 1.5);
xlabel('Year'); ylabel('Bkrnd + HIV mortality per 100K'); title('MLWHIV aged 15-64'); grid on;
xlim([2000 2020]); %ylim([0 10]);
legend('Model, HIV+ ART: 25-sets mean' , 'Model: 25-sets minimum' , 'Model: 25-sets maximum' , ...
    'Model, HIV+ no ART: 25-sets mean' , 'Model: 25-sets minimum' , 'Model: 25-sets maximum');

subplot(1 , 3 , 2)
plot(mortYearVec , mean(squeeze(mortF_hivPosART(: , :)),1)' , 'k-' , ...
    mortYearVec , min(squeeze(mortF_hivPosART(: , :)),[],1)' , 'k--' , ...
    mortYearVec , max(squeeze(mortF_hivPosART(: , :)),[],1)' , 'k--' , 'LineWidth' , 1.5);
hold all;
plot(mortYearVec , mean(squeeze(mortF_hivPosNoART(: , :)),1)' , 'b-' , ...
    mortYearVec , min(squeeze(mortF_hivPosNoART(: , :)),[],1)' , 'b--' , ...
    mortYearVec , max(squeeze(mortF_hivPosNoART(: , :)),[],1)' , 'b--' , 'LineWidth' , 1.5);
xlabel('Year'); ylabel('Bkrnd + HIV mortality per 100K'); title('WLWHIV aged 15-64'); grid on;
xlim([2000 2020]); %ylim([0 10]);
legend('Model, HIV+ ART: 25-sets mean' , 'Model: 25-sets minimum' , 'Model: 25-sets maximum' , ...
    'Model, HIV+ no ART: 25-sets mean' , 'Model: 25-sets minimum' , 'Model: 25-sets maximum');

subplot(1 , 3 , 3)
plot(mortYearVec , mean(squeeze(mortF_hivPosART(: , :)+mortM_hivPosART(: , :)),1)' , 'k-' , ...
    mortYearVec , min(squeeze(mortF_hivPosART(: , :)+mortM_hivPosART(: , :)),[],1)' , 'k--' , ...
    mortYearVec , max(squeeze(mortF_hivPosART(: , :)+mortM_hivPosART(: , :)),[],1)' , 'k--' , 'LineWidth' , 1.5);
hold all;
plot(mortYearVec , mean(squeeze(mortF_hivPosNoART(: , :)+mortM_hivPosNoART(: , :)),1)' , 'b-' , ...
    mortYearVec , min(squeeze(mortF_hivPosNoART(: , :)+mortM_hivPosNoART(: , :)),[],1)' , 'b--' , ...
    mortYearVec , max(squeeze(mortF_hivPosNoART(: , :)+mortM_hivPosNoART(: , :)),[],1)' , 'b--' , 'LineWidth' , 1.5);
xlabel('Year'); ylabel('Bkrnd + HIV mortality per 100K'); title('PLWHIV aged 15-64'); grid on;
xlim([2000 2020]); %ylim([0 10]);
legend('Model, HIV+ ART: 25-sets mean' , 'Model: 25-sets minimum' , 'Model: 25-sets maximum' , ...
    'Model, HIV+ no ART: 25-sets mean' , 'Model: 25-sets minimum' , 'Model: 25-sets maximum');
...
figure('DefaultAxesFontSize' , 18);
%subplot(1,2,1);
plot((artYr + 1) , [maxRateM(1:end-2,1);0.3978;0.3978].*100 , 'ko');
hold all;
plot([startYear:timeStep:currYear] , mean(artCovM,1)' , 'k-' , 'LineWidth' , 1.5);
hold all;
x2 = [[startYear:timeStep:currYear] , fliplr([startYear:timeStep:currYear])];
inBetween = [max(squeeze(artCovM),[],1) , fliplr(min(squeeze(artCovM),[],1))];
h = fill(x2 , inBetween , 'k');
h.FaceAlpha = 0.3;
h.LineStyle = '--';
xlim([1990 2020]); ylim([0 100]);
xlabel('Year'); grid on; %ylabel('MLWHIV aged 15-79 on ART & virally suppressed (%)');
%title('Men');
%legend('Observed KZN' , 'Modeled KZN, ages 15-79: 25-sets median' , 'Model: 25-sets range');

%subplot(1,2,2);
hold all;
plot((artYr + 1) , [maxRateF(1:end-2,1);0.6223;0.6223].*100 , 'o' , 'Color' , [0.5,0.5,0.5]);
hold all;
plot([startYear:timeStep:currYear] , mean(artCovF,1)' , '-' , 'Color' , [0.5,0.5,0.5] , 'LineWidth' , 1.5);
hold all;
x2 = [[startYear:timeStep:currYear] , fliplr([startYear:timeStep:currYear])];
...
xlabel('Year'); ylabel('PLWHIV aged 15-79 on ART & virally suppressed (%)');
grid on; %title('Women');
legend('Observed KZN, men' , 'Modeled KZN, men: mean' , 'range' , ...
    'Observed KZN, women' , 'Modeled KZN, women: mean' , 'range' , 'Location' , 'northwest');
...
%% Proportion HIV-negative males circumcised by broad age groups over time
circPropYr_obs = [vmmcYr(1:end-2,:);2017;2020];
circProp_obs = [vmmcRate(1:end-2,:);[0.459 0.420 0.318 0.204];[0.631 0.527 0.394 0.241]]' .* 100;

figure('DefaultAxesFontSize' , 18);
plot(circPropYr_obs , circProp_obs , 'o');
hold on;
set(gca,'ColorOrderIndex',1)
p = plot([startYear:timeStep:currYear] , squeeze(mean(circProp,1)) , '-' , 'LineWidth' , 1.5);
hold all;
x2 = [[startYear:timeStep:currYear] , fliplr([startYear:timeStep:currYear])];
...
legend('Observed KZN, ages 15-19' , ...
    ...
    'Modeled KZN, ages 15-19: mean' , ...
    'Modeled KZN, ages 20-24: mean' , ...
    'Modeled KZN, ages 25-49: mean'  , ...
    'Modeled KZN, ages 50+: mean' , ...
    'range' , ...
    'range' , ...
    'range' , ...
