...
% example: vaxCEA_multSims_CIs_modScreen(2 , '0' , {'1' , '0'})
...
hpv_hivMW = zeros(nRuns , 5 , age , length(monthlyTimespan));
% CIN2/3 prevalence
...
overScreenTotAnnualCum = zeros(nRuns , 5 , 3 , length(futAnnualTimespan));
...
n = vaxResultInd;
baseFileName = ['22Apr20Ph2V11_2v57BaseVax_spCytoScreen_hpvHIVcalib_adjFert2_adjCCAgeMults3_KZNCC4_noVMMChpv_SA-S' , sceNum , '_'];
...
for k = 1 : loopSegmentsLength-1
    parfor j = loopSegments{k}+1 : loopSegments{k+1}
        ...
        curr = load([pwd , '/HHCoM_Results/toNow_22Apr20Ph2V11_2v57BaseVax_spCytoScreen_hpvHIVcalib_adjFert2_adjCCAgeMults3_KZNCC4_noVMMChpv_' , fileInds{j}]); % ***SET ME***: name for historical run output file 
    ...
    %     noV = vaxResult{noVaxInd};
        ...
        for dInd = 1 : diseaseVecLength_ccInc
            ...
            for a = 1 : age
                hpvInds = unique([toInd(allcomb(d , 1 : viral , 2 : 6 , 1 : hpvNonVaxStates , ...
                    1 : 3 , 1 : intervens , 1 : gender , a , 1 : risk)); toInd(allcomb(d , 1 : viral , ...
                    1 : hpvVaxStates , 2 : 5 , 1 : 3 , 1 : intervens , 1 : gender , a , 1 : risk))]);
                ageInds = toInd(allcomb(d , 1 : viral , 1 : hpvVaxStates , 1 : hpvNonVaxStates , ...
                    1 : endpoints , 1 : intervens , 1 : gender , a , 1 : risk));
                hpv_hivMW(j , dInd , a , :) = sum(vaxResult{n}.popVec(: , hpvInds))...
                    ./ sum(vaxResult{n}.popVec(: , ageInds));
            end
        end
        ...
        for dInd = 1 : diseaseVecLength_ccInc
            d = diseaseVec_ccInc{dInd};
                % Calculate incidence
                ccAnlHivTime(j , dInd , :) = ...
                    squeeze(annlz(sum(sum(sum(vaxResult{n}.newCC(((2020 - startYear) * stepsPerYear +1):end , d , : , :),2),3),4)));
        end
        ...
        %% Total number of women overscreened annually (SUS, HPV, CIN1) by HIV disease status
        for dInd = 1 : diseaseVecLength_ccInc
            d = diseaseVec_ccInc{dInd};
            overScreenTotAnnualCum(j , dInd , : , :) = [annlz(sum(sum(sum(sum(sum(sum(vaxResult{n}.newScreen(: , d , [1,7] , [1,7] , 1 , : , :),2),3),4),5),6),7)) ; ...
                (annlz(sum(sum(sum(sum(sum(sum(vaxResult{n}.newScreen(: , d , 2 , [1:2 , 7] , 1 , : , :),2),3),4),5),6),7)) + annlz(sum(sum(sum(sum(sum(sum(vaxResult{n}.newScreen(: , d , [1 , 7] , 2 , 1 , : , :),2),3),4),5),6),7))) ; ...
                (annlz(sum(sum(sum(sum(sum(sum(vaxResult{n}.newScreen(: , d , 3 , [1:3 , 7] , 1 , : , :),2),3),4),5),6),7)) + annlz(sum(sum(sum(sum(sum(sum(vaxResult{n}.newScreen(: , d , [1:2 , 7] , 3 , 1 , : , :),2),3),4),5),6),7)))];
        end
...
for dInd = 1 : length(diseaseLabels)
    ...
    outputVec = [outputVec; ...
        [squeeze(median(squeeze(ccInc(: , : , (end-firstYrRange2):end)) , 1))']];
end
fname = [pwd , '\HHCoM_Results\Vaccine' , baseFileName , fileInds{1} , '\' , ...
    'ART_comparative_modeling_outcome_templates_020221.xlsx'];  
writematrix(outputVec , fname , 'Sheet' , ['S' , fileKeyNums{n}] , 'Range' , 'F4') 
