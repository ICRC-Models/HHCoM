...
function[dPop , ccScreen , ccTreatImm , ccTreatHpv , ccTreatHyst] = hpvScreen(pop , ...
    ...
    noVaxToScreenHyst , vaxToScreenHyst , screenAlgorithm , numScreenAge , ageMultsComb)
...
for i = 1 : length(screenAlgs)
    ...
    for aS = (prevAL + 1) : (prevAL + length(screenAlgs{i}.screenAge))
        screenRateAge = screenRate * ageMultsComb(aS); % find 1/5 of age group (represents 35 year olds, only)
