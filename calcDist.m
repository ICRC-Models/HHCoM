...
% elapsed time steps.
% Accepts artDistList, a linked list of matrices describing the distribution of individuals who
...
for i = 0 : artDistList.size() - 1 % Note: Java uses 0-based indexing
    dist = reshape(artDistList.get(i) , size(s)); % reshape artDistList ast time i to match the shape of s
    if sumall(dist) ~= 0
