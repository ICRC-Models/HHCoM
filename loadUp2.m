...
% HIV tranmission rate % make HIV M-> F trans the same 
...
% HPV progression multipliers for 5-year age groups
...
% Uncomment if trying to re-generate .mat file from excel. Will likely fail if
% multithreading
% file = [pwd , '/Config/HIV_parameters_Kenya.xlsx'];
...
cisnet.cinTreatHpvPersist = 0.28; % HPV persistence with cryotherapy including treatment failure; used only for cinTreatHpvPersist calculation
...
whob.cinTreatHpvPersist = 0.0; % not used
...
% hivPrevF_dObs = zeros(38, 3); 
% hivPrevM_dObs =  zeros(42, 3);
...
% 
% hivPrevF_dObs(: , 1) = xlsread(file , 'Calibration' , 'E52:E88'); % HIV Prevalence in Women 2003, 2007, and 2009, by age
% hivPrevF_dObs(: , 2:3) = xlsread(file , 'Calibration' , 'H52:I88');
% hivPrevM_dObs(: , 1) = xlsread(file , 'Calibration' , 'E90:E131'); % HIV Prevalence in Men 2003, 2007, and 2009, by age
% hivPrevM_dObs(: , 2:3) = xlsread(file , 'Calibration' , 'H90:I131');
% hivPrevAll_dObs(: , 1) = xlsread(file , 'Calibration' , 'E132:E141'); % HIV Prevalence in all in 2012, by age
% hivPrevAll_dObs(: , 2:3) = xlsread(file , 'Calibration' , 'H132:I141');
%  
% popAgeDist_dObs(: , 1) = xlsread(file , 'Calibration' , 'E142:E205');  % Population age distribution in 1979, 1989, 1990, and 2009
% popAgeDist_dObs(: , 2:3) = xlsread(file , 'Calibration' , 'H142:I205'); 
% 
% totPopSize_dObs(: , 1) = xlsread(file , 'Calibration' , 'E206:E209'); % Total population size in 1979, 1989, 1990, and 2009
% totPopSize_dObs(: , 2:3) = xlsread(file , 'Calibration' , 'H206:I209');
%  
...
fromVaxNoScrnInds = sort(toInd(allcomb(1 : disease , 1 : viral , 1 : hpvVaxStates , ...
    1 : hpvNonVaxStates , 1 : endpoints , 2 , 1 : gender , 1 : age , 1 : risk))); 
fromVaxScrnInds = sort(toInd(allcomb(1 : disease , 1 : viral , 1 : hpvVaxStates , ...
    1 : hpvNonVaxStates , 1 : endpoints , 4 , 1 : gender , 1 : age , 1 : risk))); 
toNonVaxNoScrnInds = sort(toInd(allcomb(1 : disease , 1 : viral , 1 : hpvVaxStates , ...
    1 : hpvNonVaxStates , 1 : endpoints , 1 , 1 : gender , 1 : age , 1 : risk))); 
toNonVaxScrnInds = sort(toInd(allcomb(1 : disease , 1 : viral , 1 : hpvVaxStates , ...
    1 : hpvNonVaxStates , 1 : endpoints , 3 , 1 : gender , 1 : age , 1 : risk)));
...
% fertility matrix for infected mothers
% disp('Building fertility4 matrix for HIV-infected mothers')
xIndsPos = [];
yIndsPos = [];
valsPos = [];
xIndsNeg = [];
