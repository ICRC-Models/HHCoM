...
pathModifier = [date , '_baseVax057_baseScreen_baseVMMC_fertDec042-076-052_2020ARTfxd_trackCD4-Discont_discontFxd_Test0107_DoART_S1_' , num2str(tstep_abc) , '_' , num2str(paramSetIdx)]; % ***SET ME***: name for simulation output file
% Directory to save results
...
vaxCoverCU = [ones(1,length(vaxAgeCU)-1).*0.50];   % coverage for catch-up vaccination by ages catch-up vaccinated % ***SET ME***: coverage for catch-up vaccination by age, *adjustment factor if fraction of 5-year cohort
...
for n = nTests
    ...
    % newScreen = zeros(length(s) - 1 , disease , viral , hpvVaxStates , hpvNonVaxStates , endpoints , numScreenAge , risk);
    ...
    for i = 2 : length(s) - 1
        ...
        [~ , pop , deaths(i , : , : , :)] = ode4xtra(@(t , pop) ...
            bornAgeDieRisk(t , pop , year , disease , ...
            gender , age , fivYrAgeGrpsOn , deathInds ,  fertMat , fertMat2 , fertMat3 , fertMat4 , ...
