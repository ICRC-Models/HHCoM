...
%% LOAD SAVED RESULTS
curr = load([pwd , '\HHCoM_Results\PAF_HIV\toNow_19May20_BaseVax_baseScreen_hpvHIVcalib_0_']); %toNow_27May20_baseVax_baseScreen_handCalibModel_baseARTvmmc % ***SET ME***: name for historical run file
...
simVec = {dirName_SCEN1 , dirName_SCEN2 , dirName_SCEN3 , dirName_SCEN4}; % , dirName_P2_SCE3 , dirName_P2_SCE4 , dirName_P2_SCE5};
% ***SET ME***: make sure the names here correspond to scenarios in simVec above
fileTits = {'SCEN1' , 'SCEN2' , 'SCEN3' , 'SCEN4'}; %{'P1_SCES12' , 'P1_SCES34' , 'P1_SCES56' , 'P2_SCE1' , 'P2_SCE2' , 'P2_SCE3' , 'P2_SCE4' , 'P2_SCE5'};
...
for j = 1 : nResults 
    ...
    nSims = size(dir([pwd , '\HHCoM_Results\PAF_HIV\' , pathModifier, '\' , '*.mat']) , 1);
    
    % ID correct file naming scheme for waning or no waning
    vaxResult = cell(nSims , 1);
    resultFileName = [pwd , '\HHCoM_Results\PAF_HIV\' , pathModifier, '\' , 'vaxSimResult'];
    if waning
        resultFileName = [pwd , '\HHCoM_Results\PAF_HIV\' , pathModifier, '\' , 'vaxWaneSimResult'];
    end
  ...
  %% CC CASE COUNTS
    ...
    for i = 1 : length(inds)
        ...
        % Save incidence
        for n = 1 : nSims
            fname = [pwd , '\HHCoM_Results\PAF_HIV' , '\' , fileTits{j} , ...
                '_Coverage' , num2str(round(vaxResult{n}.vaxRate * 100)) , '_vaxResult' , num2str(n) , '_CCcases' , '.xlsx'];
            sname = plotTits1{i};
...
%         end
    ...
    for i = 1 : length(inds)
        ...
        for n = 1 : nSims
             fname = [pwd , '\HHCoM_Results\PAF_HIV' , '\' , fileTits{j} , ...
                '_Coverage' , num2str(round(vaxResult{n}.vaxRate * 100)) , '_vaxResult' , num2str(n) , 'RawInc_ages9to74' , '.xlsx'];
            sname = plotTits1{i};
            if exist(fname , 'file') == 2
                M = xlsread(fname);
                M = catpad(2 , [tVec(1 : stepsPerYear : end)' , vaxResult{n}.ccInc'] , M);
...
%         end
    ...
    for i = 1 : length(inds)
        ...
        for n = 1 : nSims
            ...
            sname = plotTits1{i}; if exist(fname , 'file') == 2
                M = xlsread(fname); M = catpad(2 , [tVec(1 : stepsPerYear: end)' , vaxResult{n}.ccInc'] , M); 
                xlswrite(fname , M , sname)
            else
                xlswrite(fname , [tVec(1 : stepsPerYear : end)' , vaxResult{n}.ccInc'] , sname)
            end
