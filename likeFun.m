...
function negSumLogL = likeFun(ccSymp , ccTreat , stageDist_2018_dObs , ...
    startYear , stepsPerYear)  
    % cinPos2007_dObs , cin1_2010_dObs ,...
...
%% Cancer stage distribution, diagnosed cases, year 2002, Kamangar 
% propTimeSpan = [((2002 - startYear) * stepsPerYear +1) : ((2002 - startYear) * stepsPerYear +6)];
...
% mObs = [mObs ; stageDist]; 
% dMean = [dMean ; stageDist_2002_dObs(: , 1)]; 
% dVar = [dVar ; stageDist_2002_dObs(: , 2)]; 

%% Cancer stage distribution, diagnosed cases, year 2018, Mungo 2021
propTimeSpan = [((2017 - startYear) * stepsPerYear +1) : ((2020 - startYear) * stepsPerYear +6)];
local = sum(ccSymp(propTimeSpan, 1, 1:end, 1:end), "all") + sum(ccTreat(propTimeSpan, 1, 1:end, 1:end), "all"); 
regional = sum(ccSymp(propTimeSpan, 2, 1:end, 1:end), "all") + sum(ccTreat(propTimeSpan, 2, 1:end, 1:end), "all"); 
distant = sum(ccSymp(propTimeSpan, 3, 1:end, 1:end), "all") + sum(ccTreat(propTimeSpan, 3, 1:end, 1:end), "all"); 
total = local+regional+distant; 
...
mObs = [mObs ; stageDist]; 
dMean = [dMean ; stageDist_2018_dObs(: , 1)]; 
dVar = [dVar ; stageDist_2018_dObs(: , 2)]; 
