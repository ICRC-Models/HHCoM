...
dPop = zeros(size(pop));
artTreat = zeros(disease , viral , hpvVaxStates , hpvNonVaxStates , endpoints , gender , age , risk);
            ...
            for g = 1 : gender
                for a = ageSexDebut : age
                    for r = 1 : risk
                        ...
                        hivPositiveArt = hivInds(8 , 6 , h , s , x , g , a , r , :);

                        % CD4 progression for HIV-positives with acute infection
                        for v = 1 : 5
                            acuteInf = hivInds(3 , v , h , s , x , g , a , r , :);
                            dPop(acuteInf) = dPop(acuteInf) ...
                                - (muHIV(a , 2) + kCD4(g , v , 1) + treat(3 , v , g , a , r)) ... % out: disease related mortality, CD4 progression, ART coverage.
                            ...
                            artTreat(3 , v , h , s , x , g , a , r) = treat(3 , v , g , a , r) .* sumall(pop(acuteInf)); % keep track of distribution of people going on ART

                            % CD4 progression for HIV-positives advanced to decreased CD4 count
                            for d = 4 : 7
                                cd4Curr = hivInds(d , v , h , s , x , g , a , r , :);
                                cd4Prev = hivInds(d - 1 , v , h , s , x , g , a , r , :);
                                kCD4_next = 0;
                                if d ~= 7
                                ...
                                artTreat(d , v , h , s , x , g , a , r) = treat(d , v , g , a , r) .* sumall(pop(cd4Curr)); % keep track of distribution of people going on ART                    
                            end
                        end
